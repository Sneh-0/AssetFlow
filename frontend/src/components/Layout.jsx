...
function Sidebar({ user, logout, onNavigate }) {
  return (
    ...
    <div className="min-h-screen flex bg-slate-100">
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 lg:hidden">
          ...
          <div className="absolute inset-y-0 left-0 animate-slide-in-left">
            <Sidebar user={user} logout={logout} onNavigate={() => setDrawerOpen(false)} />
          </div>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <header className="lg:hidden sticky top-0 z-40 flex items-center gap-3 px-4 py-3 bg-slate-950 text-white shadow-md">
          ...
          <div className="ml-auto w-8 h-8 rounded-full bg-gradient-to-br from-indigo-400 to-violet-500 flex items-center justify-center text-[11px] font-bold">
            {initials(user.name)}
          </div>
        </header>
